...
async function evaluateSpeed(notif, packet) {
  ...
  if (useRoadSpeed) {
    ...
    thresholdSource = roadSpeedInfo.speedLimit !== null
      ? `road speed limit (${threshold} km/h, ${position?.address}, Lat: ${position?.latitude}, Lng: ${position?.longitude})`
      : `routing speed fallback (${threshold} km/h, no posted limit, Lat: ${position?.latitude}, Lng: ${position?.longitude})`;
